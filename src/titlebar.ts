...
export function getCurrentBackground(folder: vscode.WorkspaceFolder): string | undefined {
...
export async function applyTitlebar(folder: vscode.WorkspaceFolder, colors: TitlebarColors): Promise<void> {
  ...
  const unchanged = OUR_KEYS.every((k) => current[customizationKey(k)] === colors[k]);
  if (unchanged) {
    return;
  }
  await cfg.update('colorCustomizations', merged, vscode.ConfigurationTarget.Workspace);
}

/** Remove only our keys from workspace colorCustomizations. */
export async function removeTitlebar(folder: vscode.WorkspaceFolder): Promise<void> {
  const cfg = vscode.workspace.getConfiguration('workbench', folder.uri);
  const current = cfg.get<Customizations>('colorCustomizations');
  if (!current) {
    return;
  ...
  await cfg.update('colorCustomizations', value, vscode.ConfigurationTarget.Workspace);
}
...
export async function ensureTitleBarStyleCustom(): Promise<void> {
  ...
  if (choice === 'Switch & Reload') {
    await cfg.update('titleBarStyle', 'custom', vscode.ConfigurationTarget.Global);
 ...
 */
export async function readSiblingBackgrounds(worktreePaths: string[]): Promise<string[]> {
  const out: string[] = [];
  for (const wt of worktreePaths) {
    try {
      const text = await fs.readFile(path.join(wt, '.vscode', 'settings.json'), 'utf8');
      const json = parseJsonc(text) as Record<string, unknown> | null;
      const custom = json?.['workbench.colorCustomizations'] as Customizations | undefined;
 ...
 */
export async function ensureGitExclude(commonDir: string, patterns: string[]): Promise<void> {
